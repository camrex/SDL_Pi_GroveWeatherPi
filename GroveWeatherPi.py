...
# Master Version 2.5 November 10, 2016
# Master Author: SwitchDoc Labs www.switchdoc.com
# Fork Version 2.51 December 16, 2016
# Fork Author: Cameron Rex
#
...
def returnStatusLine(device, state):
    """Check attached device state and return status."""
    returnString = device
    if (state is True):
...
WXLinkResetPin = 12
SSD1306_RST = 27
as3935pin = 13  # define Interrupt Pin for AS3935
...
sunAirPlus = SDL_Pi_INA3221.SDL_Pi_INA3221(twi=SUNAIR_BUS, addr=0x40)
...
################
try:
    Sunlight_Sensor = SDL_Pi_SI1145.SDL_Pi_SI1145(busnum=SI1145_BUS)
...
ds3231 = SDL_DS3231.SDL_DS3231(DS3231_BUS, 0x68)
try:
    ds3231.write_now()
...
try:
    display = Adafruit_SSD1306.SSD1306_128_64(rst=SSD1306_RST, i2c_bus=SSD1306_BUS, i2c_address=0x3C)
    # Initialize library.
    display.begin()
    display.clear()
    display.display()
...
if (config.Lightning_Mode is True):
    as3935 = RPi_AS3935(address=0x03, bus=AS3935_BUS)
    try:
        ...
        if (config.AS3935_Present is True):
            # i2ccommand = "sudo i2cdetect -y 1"
...
###############
# Set up FRAM
fram = SDL_Pi_FRAM.SDL_Pi_FRAM(addr=0x50, twi=FRAM_BUS)
# FRAM Detection
try:
        fram.read8(0)
...
def writeSunAirPlusStats():  # write SunAirPlus stats out to file
...
def writeWeatherStats():  # write weather stats out to file
...
def sampleWeather():  # sample and display
    ...
    print "----------------- "
    if (config.WXLink_Present is False):
        ...
        if ((config.ADS1015_Present is True) or (config.ADS1115_Present is True)):
    ...
    if (config.WeatherUnderground_Present is True):
...
def sampleAndDisplay():
    ...
    print "----------------- "
    print "----------------- "
    if (config.DS3231_Present is True):
    ...
    print "----------------- "
    print "----------------- "
    if (config.BMP280_Present is True):
    ...
    print "----------------- "
    if (config.Sunlight_Present is True):
    ...
    if (config.Sunlight_Present is True):
        ...
        print "----------------- "
    if (config.HTU21DF_Present is True):
    ...
    print "----------------- "
    print "----------------- "
    if (config.AM2315_Present is True):
...
def checkInternetConnection():
    """Check internet connection by testing google.com."""
    try:
...
def WLAN_check():
    """
    Check if the WLAN is still up by pinging the router.
    ...
    if (config.enable_WLAN_Detection is True):
...
if (config.SunAirPlus_Present is False):
    batteryCurrent = 0.0
    batteryVoltage = 4.00
    batteryPower = batteryVoltage * (batteryCurrent / 1000)
    solarCurrent = 0.0
